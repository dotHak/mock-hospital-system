...
import { eq, and, or, sql, lt, lte, gt, gte, not } from "drizzle-orm";
...
export const create: AppRouteHandler<CreateRouteType> = async (c) => {
    ...
    const startDate = new Date(values.appointmentDate).setHours(startHour, startMinute);
    ...
    const slotsCounts = await db.transaction(async (tx) => {
        const aCounts = await db.$count(
            ...
            and(
                ...
                or(
                    and(
                        lte(sql`time(${appointments.startTime})`, sql`time(${values.startTime})`),
                        gt(sql`time(${appointments.endTime})`, sql`time(${values.startTime})`),
                    ),
                    and(
                        lt(sql`time(${appointments.startTime})`, sql`time(${values.endTime})`),
                        gte(sql`time(${appointments.endTime})`, sql`time(${values.endTime})`),
                    ),
                    and(
                        gt(sql`time(${appointments.startTime})`, sql`time(${values.startTime})`),
                        lt(sql`time(${appointments.endTime})`, sql`time(${values.endTime})`),
                    ),
                ),
            ),
        );
...
export const update: AppRouteHandler<UpdateRouteType> = async (c) => {
    ...
    if (
        ...
        return c.json(
            {
                message:
                    "You cannot update appointment date, start time, end time or doctor, please cancel and create a new appointment",
                success: false,
                code: HttpStatusCodes.UNPROCESSABLE_ENTITY,
            },
            HttpStatusCodes.UNPROCESSABLE_ENTITY,
